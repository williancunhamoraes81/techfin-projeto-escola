...
@OpenAPIDefinition(info = @Info(title = "API Turma POC TechFin", description = "Controle de Alunos", version = "1.0", contact = @Contact(name = "", url = "", email = "")), servers = @Server(url = "{{host}}/api/v1/alunos", description = "Descrição do Server", variables = {
		@ServerVariable(name = "serverUrl", defaultValue = "localhost"),
		@ServerVariable(name = "serverHttpPort", defaultValue = "8080") }))
@XTOTVSApi(messageDocumentation = @MessageDocumentationApi(name = "Product", description = "Produto", segment = "Saúde"), productInformation = @ProductInformationApi(product = "OpenApiSample", contact = "tjf@totvs.com.br", description = "Open Api Sample", helpUrl = "http://tjf.totvs.com.br"))
public class AlunoResourceTOTVS {
	...
	@Operation(summary = "Buscar Alunos", description = "Buscar lista de Alunos", operationId = "AlunoGet", tags = {
	"client" }, responses = { @ApiResponse(responseCode = "200", description = "Lista de alunos retornada."),
			@ApiResponse(responseCode = "400", description = "Erro ao retornar a lista de alunos.") })
	@GetMapping()
	@XTOTVSEndPoint(productInformation = @ProductInformationEndPoint(minimalVersion = "2.0", product = "OpenApiSample"))
	public ApiCollectionResponse<Aluno> findAll() {
		return ApiCollectionResponse.of(repository.findAll());
	}

	@Operation(summary = "Buscar Aluno por Id", description = "Buscar Aluno por Id", operationId = "produtoGet", tags = {
			"client" }, responses = { @ApiResponse(responseCode = "200", description = "Aluno retornado."),
	...
	public ResponseEntity<Aluno> insert(@Valid @RequestBody Aluno obj) {
		service.insert(obj);
		URI locationURI = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}").buildAndExpand(obj.getId())
				.toUri();
